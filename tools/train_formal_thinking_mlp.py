...
def main():
    ...
    manifest = load_data_manifest(args.data_manifest, require_formal=args.formal)
    ...
    datasets = {s: CanonicalThinkingDataset(manifest["splits"][s]["path"], tokenizer,
    ...
    (out/"training_manifest.json").write_text(json.dumps({"complete":True,"best_validation_mse":best,"optimizer_step":step,"data_manifest":args.data_manifest,"max_thinking_tokens":cfg["max_thinking_tokens"],"observed_max_thinking_length":max(dataset.max_thinking_length for dataset in datasets.values())},indent=2))
