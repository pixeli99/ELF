...
from src.utils.formal_thinking_mlp import (CanonicalThinkingDataset,StreamingChannelMoments,
 ...
 load_frozen_encoder_artifact,load_trusted_full_training_checkpoint,
 mean_pool_reconstruction,refuse_existing,whiten)
...
class FormalThinkingMLPTests(unittest.TestCase):
 ...
 def test_canonical_x0_keyword_contract_and_normalized_return(self):
  ...
  with mock.patch("src.utils.encoder_utils.encode_text",return_value=normalized) as helper:
 ...
 def test_forward_length_gate_ignores_padding(self):
  ...
  with mock.patch("src.utils.encoder_utils.encode_text",return_value=torch.empty(0)) as helper:
   canonical_t5_x0(encoder,ids,mask,0.0,0.2);self.assertEqual(helper.call_count,1)
  mask[:,:]=1
  with mock.patch("src.utils.encoder_utils.encode_text") as helper:
