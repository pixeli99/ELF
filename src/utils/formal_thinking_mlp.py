...
class ShardedThinkingDataset(Dataset):
    """Lazy train-only dataset; model batches expose thinking IDs and masks only."""
    def __init__(self, shard_paths, tokenizer, max_thinking_tokens: int = 1024):
        self.tokenizer = tokenizer
        self.max_thinking_tokens = int(max_thinking_tokens)
        self.rows = []
        self.max_thinking_length = 0
        for path in map(Path, shard_paths):
            with path.open("rb") as handle:
                while True:
                    offset = handle.tell(); line = handle.readline()
                    if not line: break
                    row = json.loads(line); thinking = row.get("thinking")
                    if not isinstance(thinking, str) or not thinking.strip():
                        raise ValueError(f"missing/empty thinking: {row.get('sample_id')}")
                    length = int(row.get("thinking_tokens", 0))
                    if not 0 < length <= self.max_thinking_tokens:
                        raise ValueError(f"invalid thinking length for {row.get('sample_id')}: {length}")
                    self.max_thinking_length = max(self.max_thinking_length, length)
                    self.rows.append((str(path), offset, str(row.get("sample_id"))))
    def __len__(self): return len(self.rows)
    def __getitem__(self, index):
        path, offset, expected_id = self.rows[index]
        with open(path, "rb") as handle:
            handle.seek(offset); row = json.loads(handle.readline())
        if str(row.get("sample_id")) != expected_id:
            raise RuntimeError("canonical shard changed after dataset indexing")
        ids = thinking_token_ids(self.tokenizer, row["thinking"], add_special_tokens=True,
                                 truncation=False)
        if len(ids) > self.max_thinking_tokens:
            raise ValueError(f"thinking length changed for {expected_id}: {len(ids)}")
        return {"sample_id": expected_id, "thinking_input_ids": ids}
...
def load_data_manifest(path: str, require_formal: bool = False,
                       train_only: bool = False) -> Dict[str, object]:
    manifest = json.loads(Path(path).read_text(encoding="utf-8"))
    if require_formal and (manifest.get("training_ready") if train_only else manifest.get("formal_ready")) is not True:
        raise ValueError("data manifest does not declare the requested training protocol ready")
    for split in (("train",) if train_only else ("train", "validation")):
        entry = manifest.get("splits", {}).get(split)
        paths = entry.get("shards", []) if entry else []
        if entry and entry.get("path"): paths = [entry["path"]]
        if not paths or not all(Path(p).is_file() for p in paths):
            raise ValueError(f"manifest has no readable {split} split")
        if entry.get("sha256") and sha256_file(paths[0]) != entry["sha256"]:
...
def canonical_t5_x0(encoder, input_ids, attention_mask, latent_mean, latent_std,
    ...
    from src.utils.encoder_utils import encode_text_components
